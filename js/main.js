      ...
      for (let i = 0; i < data.data.length; i++) {
        ...
        getUl.innerHTML += `<li class="weather_print">
                            <div class="weather_contents">
                                <img src=https://www.weatherbit.io/static/img/icons/${data.data[i].weather.icon}.png alt="" class="forecast_img" />
                                    <span class="weather_temp">${Math.round(data.data[i].max_temp) + "&#8457;"}</span>
                            </div>
                            <div class="weather_float">
                                <span class="weather_time">${days[workPls]}</span>
                                <span class="weather_day">${data.data[i].datetime}</span>
                            </div>
                            </li>
                            </ul>`;
        
        console.log(days[workPls]);
      }
